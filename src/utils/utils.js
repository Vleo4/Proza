...
const getSearchTermVideos = async (queryString, searchResultsSetterFunction, useLocalStorage) => {
    try {
...
export const handleSearchFormSubmit = (
    ...
    if (useLocalStorage && storedResults) {
        ...
        getSearchTermVideos(queryString, searchResultsSetterFunction, useLocalStorage);
    }
...
export const useGetChannelDetails = (
    ...
    useEffect(() => {
        ...
        if (useLocalStorage) {
            if (isVideo) {
                storedChannelAvatar = JSON.parse(localStorage.getItem(`${videoId}_channelAvatar`));
            } else {
                storedChannelInfo = JSON.parse(localStorage.getItem(`${channelId}_channelInfo`));
            }
        }
...
export const getFormattedDurationString = (duration) => {
    ...
    // remove leading '0'
    formattedDuration =
        formattedDuration[0] === '0' ? formattedDuration.slice(1) : formattedDuration;
...
export function HideOnScroll({ children }) {
    ...
    const displayDesktopHeader = !useMediaQuery(`(max-width:${MOBILE_VIEW_MAX_WIDTH}px)`);
    // always show Header in desktop view
    const showHeader = displayDesktopHeader ? displayDesktopHeader : displayMobileHeader;
